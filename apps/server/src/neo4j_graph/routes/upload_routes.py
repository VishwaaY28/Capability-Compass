...
# ---------------------------------------------------------------------------
# Human-in-the-Loop (HITL) ingestion session manager
#
# The Compass UI now drives ingestion as a 3-step wizard so a human can
# review the FIBO document gate, then the raw LLM extraction, then the
# post-extraction guardrail before anything lands in Neo4j. Each session
# tracks the state required for those steps in process memory; the temp
# upload file lives on disk until the session is finalised or cancelled.
#
# Sessions are kept in process memory only (no DB) because the wizard is
# an interactive single-process workflow and the data can always be
# regenerated by re-uploading the document. A simple TTL-based janitor
# cleans up sessions abandoned for more than `SESSION_TTL_SECONDS`.
# ---------------------------------------------------------------------------
...
def _gc_sessions() -> None:
    """Drop expired sessions and clean up their temp files."""
    now = time.time()
    expired_ids: List[str] = []
    with _sessions_lock:
        for sid, sess in _sessions.items():
            if now - sess.get("updated_at", now) > SESSION_TTL_SECONDS:
                expired_ids.append(sid)
        for sid in expired_ids:
            sess = _sessions.pop(sid, None)
            if sess:
                tmp_path = sess.get("tmp_path")
                if tmp_path and os.path.exists(tmp_path):
                    try:
                        ...
                        pass
    if expired_ids:
        logger.info(f"[HITL] GC'd {len(expired_ids)} expired session(s)")
...
def _delete_session(session_id: str, *, cleanup_tmp: bool = True) -> None:
    with _sessions_lock:
        sess = _sessions.pop(session_id, None)
    if sess and cleanup_tmp:
        tmp_path = sess.get("tmp_path")
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.unlink(tmp_path)
            except Exception as exc:
                logger.warning(f"[HITL] Could not remove temp file {tmp_path}: {exc}")


def _serialize_doc_gate(doc_gate: Dict[str, Any]) -> Dict[str, Any]:
    """Strip non-JSON-friendly fields from the doc-gate payload for the UI."""
    return {
        "available": doc_gate.get("available"),
        "is_relevant": doc_gate.get("is_relevant"),
        "rejection_reason": doc_gate.get("rejection_reason"),
        "ontology_meta": doc_gate.get("ontology_meta"),
        "relevance": doc_gate.get("relevance"),
        "error": doc_gate.get("error"),
    }
...
# ---------------------------------------------------------------------------
# HITL endpoints — the 3-step ingestion wizard
# ---------------------------------------------------------------------------
...
async def start_hitl_session(
    ...
    """Stage 1: upload a document, run the FIBO document gate, return evidence.

    The document is parsed and chunked, then scored against the FIBO
    ontology *without* running the LLM extractor. A new ingestion session
    is created; the response carries everything the UI needs to show:

    - the FIBO ontology metadata,
    - the document-level relevance verdict (with top concepts and excerpts),
    - the matching ("passed") chunks the user can review.

    The session id returned must be passed to subsequent stages.
    """
    try:
        from utils.deepagent_extractor import prepare_chunks_and_gate
        ...
        logger.info(
            f"[HITL] /session/start file='{file.filename}' tmp='{tmp_path}' "
            f"vertical={vertical!r} subvertical={subvertical!r} "
            f"capability={capability!r} depth={extraction_depth!r}"
        )

        try:
            stage1 = prepare_chunks_and_gate(
                file_path=tmp_path,
                extraction_depth=extraction_depth,
                skip_embeddings=False,
            )
        except Exception:
            # Clean up the tmp file if stage 1 itself blows up.
            try:
                os.unlink(tmp_path)
            except Exception:
                pass
        ...
        session = {
            ...
            "chunk_count": stage1["chunk_count"],
            "chunks_path": stage1["chunks_path"],
            "doc_gate": stage1["doc_gate"],
            "is_relevant": stage1["is_relevant"],
            "rejection_reason": stage1["rejection_reason"],
            "evidence_chunks": stage1["evidence_chunks"],
            "extracted_data": None,
        ...
        # If the doc gate already rejected the document, surface that as
        # the session's terminal state so the UI can short-circuit step 2.
        ontology_status = (
            "document_rejected"
            if (
                stage1["doc_gate"].get("available")
                and not stage1["is_relevant"]
            )
            else "passed_doc_gate"
        )
        ...
        return {
            ...
            "chunk_count": stage1["chunk_count"],
            "chunks_path": stage1["chunks_path"],
            "doc_gate": _serialize_doc_gate(stage1["doc_gate"]),
            "ontology": stage1["doc_gate"].get("ontology_meta"),
            "document_relevance": stage1["doc_gate"].get("relevance"),
            "is_relevant": stage1["is_relevant"],
        ...
        logger.error(f"[HITL] /session/start failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/session/{session_id}/extract")
async def extract_processes_for_session(session_id: str):
    """Stage 2: run the LLM extractor on the session's chunks.

    Returns the *raw* extracted capability model (no FIBO guardrail yet).
    The UI shows this so the user can review the LLM output before the
    ontology guardrail trims it down in stage 3.
    """
    sess = _get_session(session_id)

    if not sess.get("is_relevant"):
        # The doc gate rejected this document. We refuse to spend an LLM
        # call on it; the UI should short-circuit, but be defensive.
        raise HTTPException(
            status_code=400,
            detail=(
                "Cannot run LLM extraction: this document failed the FIBO "
                "document gate. Reason: "
                f"{sess.get('rejection_reason') or 'unknown'}"
            ),
        )

    if sess["stage"] not in ("doc_gated", "extracted"):
        raise HTTPException(
            status_code=400,
            detail=(
                f"Session is in stage '{sess['stage']}' which does not allow "
                "extraction. Start a new session."
            ),
        )

    try:
        from utils.deepagent_extractor import run_llm_extraction

        cfg = sess["config"]
        extracted = run_llm_extraction(
            chunks=sess["chunks"],
            doc_gate=sess["doc_gate"],
            vertical=cfg.get("vertical"),
            subvertical=cfg.get("subvertical"),
            capability=cfg.get("capability"),
            extraction_depth=cfg.get("extraction_depth", "data_element"),
            file_hash=sess.get("file_hash"),
        )
    except Exception as e:
        logger.error(f"[HITL] /session/{session_id}/extract failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"LLM extraction failed: {e}")

    sess["extracted_data"] = extracted
    sess["stage"] = "extracted"
    sess["updated_at"] = time.time()

    process_count = len(extracted.get("processes") or [])
    subprocess_count = sum(
        len(p.get("subprocesses") or []) for p in extracted.get("processes") or []
    )
...
@router.post("/session/{session_id}/guardrail")
async def apply_guardrail_for_session(session_id: str):
    """Stage 3: apply the FIBO post-extraction guardrail to the LLM output.

    Returns the guardrail-filtered model (only accepted processes), the
    full guardrail summary (so the UI can show which were rejected and
    why), and the final ontology status.
    """
    sess = _get_session(session_id)

    if not sess.get("extracted_data"):
        raise HTTPException(
            status_code=400,
            detail=(
                "No extracted data on this session yet. "
                "Run /session/{id}/extract first."
            ),
        )

    try:
        from utils.deepagent_extractor import apply_guardrail_to_extraction

        outcome = apply_guardrail_to_extraction(
            extracted_data=sess["extracted_data"],
            doc_gate=sess.get("doc_gate"),
        )
    except Exception as e:
        logger.error(f"[HITL] /session/{session_id}/guardrail failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Ontology guardrail failed: {e}")

    sess["guardrail_outcome"] = outcome
    sess["stage"] = "guardrailed"
    sess["updated_at"] = time.time()
    ...
    return {
        ...
        "session_id": session_id,
        "stage": "guardrailed",
        "annotated_data": outcome["annotated_data"],
        "ontology": outcome["ontology"],
        "guardrail": outcome["guardrail"],
        "ontology_status": outcome["ontology_status"],
        "document_relevance": (sess.get("doc_gate") or {}).get("relevance"),
        "chunks_path": sess.get("chunks_path"),
    }


@router.post("/session/{session_id}/import")
async def import_session_to_graph(
    session_id: str,
    selection: Optional[Dict[str, Any]] = Body(None),
):
    """Stage 4: persist the guardrail-approved model into Neo4j.

    The session's ``annotated_data`` and the on-disk chunks file are
    forwarded to the existing ``_import_model_to_neo4j`` helper. On
    success the session is finalised and its temp upload file is
    cleaned up.

    Optional request body (lets the user trim the guardrail-approved
    model before persistence, e.g. via checkboxes in the UI)::

        {
          "selected_process_indices": [0, 2, 3],
          "selected_subprocess_indices": {"0": [0, 1], "2": [0]}
        }

    Indices refer to positions in ``annotated_data["processes"]`` (and
    each process's ``subprocesses``) as returned by the guardrail stage.
    If ``selected_process_indices`` is omitted/null all processes are
    imported. If a process is selected but no entry exists for it in
    ``selected_subprocess_indices`` all of its subprocesses are imported.

    The selection can only *narrow* the guardrail output — the backend
    never imports a process the guardrail did not already accept.
    """
    sess = _get_session(session_id)
    outcome = sess.get("guardrail_outcome")
    if not outcome or sess["stage"] != "guardrailed":
        raise HTTPException(
            status_code=400,
            detail=(
                "Session has not run the guardrail yet. "
                "Run /session/{id}/guardrail first."
            ),
        )

    annotated = outcome.get("annotated_data") or {}
    if not annotated.get("processes"):
        raise HTTPException(
            status_code=400,
            detail=(
                "The guardrail rejected every extracted process; nothing to import. "
                "Tweak the source document or thresholds and start a new session."
            ),
        )

    try:
        filtered = _apply_process_selection(annotated, selection)
    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            f"[HITL] /session/{session_id}/import selection filtering failed: {e}",
            exc_info=True,
        )
        raise HTTPException(status_code=400, detail=f"Invalid selection payload: {e}")

    if not filtered.get("processes"):
        raise HTTPException(
            status_code=400,
            detail=(
                "Your selection excluded every process — nothing to import. "
                "Pick at least one process and try again."
            ),
        )

    try:
        summary = await _import_model_to_neo4j(
            filtered, sess.get("chunks_path")
        )
        try:
            from utils.llmthinking import azure_openai_thinking_client
            azure_openai_thinking_client.refresh_catalog()
        except Exception as e:
            logger.warning(f"Catalog refresh after HITL import failed (non-critical): {e}")
    except Exception as e:
        logger.error(f"[HITL] /session/{session_id}/import failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Import failed: {e}")

    sess["stage"] = "completed"
    sess["updated_at"] = time.time()
    _delete_session(session_id, cleanup_tmp=True)
...
async def cancel_hitl_session(session_id: str):
    """Discard a HITL ingestion session and clean up its temp file."""
    _delete_session(session_id, cleanup_tmp=True)
    return {"status": "success", "session_id": session_id, "stage": "cancelled"}


@router.get("/session/{session_id}")
async def get_hitl_session(session_id: str):
    """Return a JSON-friendly snapshot of the current session state."""
    sess = _get_session(session_id)
    return {
        "status": "success",
        "session_id": session_id,
        ...
        "chunks_path": sess.get("chunks_path"),
        "doc_gate": _serialize_doc_gate(sess.get("doc_gate") or {}),
        "is_relevant": sess.get("is_relevant"),
        "rejection_reason": sess.get("rejection_reason"),
        "evidence_chunks": sess.get("evidence_chunks"),
        "extracted_data": sess.get("extracted_data"),
        "guardrail_outcome": (
            {
                k: v
                for k, v in (sess.get("guardrail_outcome") or {}).items()
                if k != "available"
            }
            if sess.get("guardrail_outcome")
            else None
        ),
    }
...
def _apply_process_selection(
    model_data: dict, selection: Optional[Dict[str, Any]]
) -> dict:
    """Return a copy of ``model_data`` filtered by the user's checkbox selection.

    ``selection`` is the optional body documented on the import endpoint::

        {
          "selected_process_indices": [int, ...],
          "selected_subprocess_indices": {"<proc_idx>": [int, ...]}
        }

    - If ``selection`` is ``None`` (or both keys are missing), the input
      is returned unchanged.
    - ``selected_process_indices = []`` deselects every process.
    - ``selected_subprocess_indices`` only filters subprocesses for the
      processes it lists; processes without an entry keep all of theirs.
    - Out-of-range indices are silently ignored.
    """
    if not selection:
        return model_data

    process_indices = selection.get("selected_process_indices")
    subprocess_indices = selection.get("selected_subprocess_indices") or {}

    if process_indices is None and not subprocess_indices:
        return model_data

    if process_indices is not None and not isinstance(process_indices, list):
        raise ValueError("selected_process_indices must be a list of integers")
    if not isinstance(subprocess_indices, dict):
        raise ValueError("selected_subprocess_indices must be an object")

    processes = list(model_data.get("processes") or [])
    if process_indices is None:
        kept_proc_idx = list(range(len(processes)))
    else:
        kept_proc_idx = [
            int(i) for i in process_indices if isinstance(i, int) and 0 <= i < len(processes)
        ]

    new_processes = []
    for proc_idx in kept_proc_idx:
        proc = dict(processes[proc_idx])
        sub_filter = subprocess_indices.get(str(proc_idx))
        if sub_filter is not None:
            if not isinstance(sub_filter, list):
                raise ValueError(
                    f"selected_subprocess_indices['{proc_idx}'] must be a list"
                )
            sub_list = list(proc.get("subprocesses") or [])
            keep_sub_idx = {
                int(i) for i in sub_filter if isinstance(i, int) and 0 <= i < len(sub_list)
            }
            proc["subprocesses"] = [
                sp for i, sp in enumerate(sub_list) if i in keep_sub_idx
            ]
        new_processes.append(proc)

    filtered = dict(model_data)
    filtered["processes"] = new_processes
    return filtered
...
async def _import_model_to_neo4j(model_data: dict, chunks_path: Optional[str] = None) -> dict:
    ...
    try:
        ...
        if chunks_path and os.path.exists(chunks_path):
            try:
                import json
                # Chunks files are written as UTF-8 by `save_document_chunks`,
                # but older runs (or files copied from a Windows-default
                # editor) might be cp1252; fall back gracefully.
                try:
                    with open(chunks_path, 'r', encoding='utf-8') as f:
                        chunks_data = json.load(f)
                except UnicodeDecodeError:
                    logger.warning(
                        f"[Import] {chunks_path} is not valid UTF-8, "
                        "retrying with cp1252"
                    )
                    with open(chunks_path, 'r', encoding='cp1252', errors='replace') as f:
                        chunks_data = json.load(f)
